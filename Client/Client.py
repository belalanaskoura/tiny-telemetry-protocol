...
def construct_header(device_id, seq_num, msg_type, batch_flag=0, checksum=0, version=1):
    timestamp = int(time.time())
    return struct.pack(HEADER_FORMAT, seq_num, device_id, msg_type, timestamp, batch_flag, checksum, version)


# Send INIT packet
init_header = construct_header(DEVICE_ID, seq_num, MSG_INIT, version= 1)
client_socket.sendto(init_header, (SERVER_IP, SERVER_PORT))
print("[INIT] sent to server", flush= True)
...
while time.time() - start_time < DURATION:
    seq_num += 1
    header = construct_header(DEVICE_ID, seq_num, MSG_DATA, batch_flag= 0, checksum= 0, version= 1)
